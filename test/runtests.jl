...
@testset "CoreNNLS.jl - Complete Forensic Suite" begin
    ...
    @testset "1. Interior & Boundary Basics" begin
        # Ax=b ist konsistent mit positiven Werten: x = [16/7, 10/7]
        @test nnls([1.0 0.5; 0.5 2.0], [3.0, 4.0]) ≈ [16/7, 10/7]
        
        # Projektion auf die Grenze (x2 muss 0 werden)
        x_boundary = nnls([1.0 0.0; 0.0 1.0], [1.0, -1.0])
        @test x_boundary ≈ [1.0, 0.0] atol=1e-12
        
        # Alles negativ -> Null-Lösung
    ...
    @testset "2. Reference: SciPy/Netlib (C/Fortran Backend)" begin
        ...
        # TestCase 3: Rank-Deficient Boundary via KKT
        ...
        for i in 1:3
            x_rank[i] < 1e-10 && @test w_rank[i] <= 1e-8 
        end
    end
    ...
    @testset "3. Reference: Scaling Stability" begin
        ...
        # Extremes Scaling (1e10): Darf nicht abstürzen
    ...
    @testset "4. Zero Column (Null Column)" begin
        ...
        @test status == :Success
        @test x[1] ≈ 1.5 && x[2] ≈ 0.0 atol=1e-16 
    end
    ...
    @testset "5. Collinearity & Rank Deficiency" begin
        ...
        status, x = nnls!(ws, A, b)
        @test status == :Success
        @test x[1] + x[2] ≈ 2.0 # Summe muss stimmen
        @test all(x .>= -1e-12)
    end
    ...
    @testset "6. Problem 'Turkey' (Ill-Conditioned)" begin
        ...
        x = nnls(A, b)
        @test all(x .>= -1e-12)
        # Bei extremer Instabilität ist das Residuum die einzig wahre Metrik
        r = b - A*x
    ...
    # ==========================================================================
    # SECTION 8: PERFORMANCE & SAFETY (LTS / SLSQP COMPLIANCE)
    # ==========================================================================
    @testset "8. In-Place & Allocation (Low-Allocation Check)" begin
        ...
        # Erlaubt Buffer für Coverage-Overhead in CI
        @test allocs < 64000 
    end
    ...
    @testset "9. Interface Safety & Complementarity" begin
        ...
        nnls!(ws, A, b)
        # Complementarity: x .* w ≈ 0
        @test all(abs.(ws.x .* ws.w) .< 1e-8)
    end

    # ==========================================================================
    # SECTION 10: MASS STRESS TEST (RANDOMIZED KKT)
    # ==========================================================================
    ...
    # ==========================================================================
    # SECTION 11-13: HISTORICAL & EXTERNAL BENCHMARKS
    # ==========================================================================
    @testset "11. R nnls Package Example (Exponential Decay)" begin
        t = collect(0:0.04:2.0)
        A = hcat([exp.(-[0.5, 0.6, 1.0][i] * t) for i in 1:3]...)
        Random.seed!(3312)
        # Simulation von Spektraldaten mit Rauschen
        b = A * [0.0, 0.7, 0.7] + 0.01 * randn(length(t))
        x = nnls(A, b)
        @test norm([0.0, 0.7, 0.7] - x) < 0.2 # Lockere Toleranz wegen Noise
    end

    @testset "12. Netlib BVLS Adapted to NNLS" begin
        Random.seed!(123)
        for case in 1:6
            ...
            x = nnls(A, b)
            w = A' * (b - A*x)
            # Validierung über Optimalitätsbedingungen
            for j in 1:n
    ...
    @testset "13. Burkardt/Lawson Fortran Synthetic Tests" begin
        ...
        for anoise in [0.0, 1e-4]
            for m in 3:6, n in 2:4
                ...
                # KKT-Check: Entscheidend für unterbestimmte Systeme (n > m)
                w = A' * (b - A * x)
                for j in 1:n
                    if x[j] > 1e-7
                        @test abs(w[j]) < 1e-4 
                ...
                # Identität nur bei überbestimmten, sauberen Systemen prüfen
